...
/// anchors on to find Submit). The LAST visible match wins: if clicking Skip
...
fn system_downloads_dir() -> Result<std::path::PathBuf> {
    // On Linux the XDG download dir can be unset or disabled (set to "$HOME/"),
    // making `download_dir()` return None even though Chromium falls back to
    // `~/Downloads` in that case -- mirror that fallback here.
    directories::UserDirs::new()
        .and_then(|u| {
            u.download_dir()
                .map(|p| p.to_path_buf())
                .or_else(|| Some(u.home_dir().join("Downloads")))
        })
        .filter(|p| p.is_dir())
